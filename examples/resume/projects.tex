...
%---------------------------------------------------------
  \cventry
    {FPGA Project Developer} % Job title
    {Digital Stopwatch Design on FPGA} % Organization
    {} % Location
    {Dr. Elizabeth Rufus | Sep. - Dec. 2021} % Date(s)
    {\begin{cvitems}
      \item {Developed a \textbf{2-digit digital stopwatch} on an \textbf{FPGA} using \textbf{Verilog HDL}, integrating clock divider, counter, and 7-segment modules; simulated on \textbf{Multisim} and synthesized via \textbf{Xilinx Vivado}.}
      \item {Verified hardware operation with accurate 1-s pulse timing and 00–99 display through a \textbf{modular HDL design}.}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {FASTAPI · NEXT.JS · REACT · NEO4J · DOCKER COMPOSE · SERVER‑SENT EVENTS · TYPESCRiPT · PYDANTiC} % Job title
    {Real‑Time Full‑Stack Knowledge Graph System} % Organization
    {}% {Vellore, S.Korea} % Location
    {August 2025 - January 2026} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Designed a decoupled microservices pipeline using Apache Kafka and Ray Serve, separating I/O‑bound video ingestion from GPU‑
intensive inference, achieving sub‑50ms inter‑service message latency and enabling independent scaling of each stage.}
        \item {Deployed a high‑throughput LLM inference service using vLLM with PagedAttention, achieving 4x throughput improvement over
naive HuggingFace inference by eliminating GPU memory fragmentation and enabling efficient KV‑cache management under concur‑
rent batching.}
        \item {Built an asynchronous drift detection service using ResNet50 feature embeddings, computing distribution shifts in real time and expos‑
ing live data quality metrics via Prometheus and Grafana, without impacting primary API latency.}
      \end{cvitems}
    }
